...
export function Certificate({ result, userName, date }: CertificateProps) {
    ...
    const printCertificate = () => {
        ...
        localStorage.setItem('printCertificate', JSON.stringify(certData));
        
        // Открываем новое окно для печати
        const printWindow = window.open('/print-certificate', '_blank', 'width=1000,height=800');
        if (!printWindow) {
            ...
            <div style={{ marginBottom: 20, textAlign: 'center' }}>
                ...
                >
                    {isDownloading ? (
                        ...
                        <>📄 Скачать / Распечатать сертификат (PDF)</>
                    )}
                </button>
            </div>

            {/* Сертификат для отображения на странице */}
            <div ref={certificateRef} className={`res-cert ${visible ? "res-cert--visible" : ""}`}>
